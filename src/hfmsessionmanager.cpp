    ...
    , m_dontShowChecked(false)
{
    ...
    const QString text(i18n("HybridFM did not close correctly. Would you like to restore these previous sessions?"));
    ...
    , m_sessionConfig(nullptr)
{
    ...
    const int interval = HybridFMSettings::autoSaveInterval();
    if (interval > 0) {
...
bool KonqSessionManager::askUserToRestoreAutosavedAbandonedSessions()
{
    ...
    const QLatin1String dontAskAgainName("Restore session when HybridFM didn't close correctly");

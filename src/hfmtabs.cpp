      ...
      m_rightWidget(nullptr), m_leftWidget(nullptr), m_alwaysTabBar(false), m_forceHideTabBar(false)
{
    ...
    m_permanentCloseButtons = HybridFMSettings::permanentCloseButton();
    if (m_permanentCloseButtons) {
        setTabsClosable(true);
    }
    tabBar()->setSelectionBehaviorOnRemove(
        HybridFMSettings::tabCloseActivatePrevious() ? QTabBar::SelectPreviousTab : QTabBar::SelectRightTab);

    if (HybridFMSettings::tabPosition() == QLatin1String("Bottom")) {
    ...
    if (HybridFMSettings::addTabButton()) {
    ...
    if (HybridFMSettings::closeTabButton()) {
...
{
    if (HybridFMSettings::mouseMiddleClickClosesTab()) {
        QTabBar *bar = tabBar();
        if (watched == bar &&

...
#include "hfmmouseeventfilter.h"
#include "hfmclosedwindowsmanager.h"
#include "hfmsessionmanager.h"
#include "hfmsessiondlg.h"
#include "hfmdraggablelabel.h"
#include "hfmcloseditem.h"
#include "hfmapplication.h"
#include "hfmguiclients.h"
...
#include "hfmframestatusbar.h"
#include "hfmtabs.h"
#include "hfmactions.h"
#include "hfmsettingsxt.h"
#include "hfmextensionmanager.h"
#include "konqueror_interface.h"
#include "delayedinitializer.h"
#include "hfmextendedbookmarkowner.h"
#include "hfmframevisitor.h"
#include "hfmbookmarkbar.h"
#include "hfmundomanager.h"
#include "hfmhistorydialog.h"
    ...
    , m_isPopupWithProxyWindow(false)
{
    ...
    if (!s_pCompletion) {
        ...
        int mode = HybridFMSettings::settingsCompletionMode();
    ...
    setXMLFile(QStringLiteral("hybrid-fm.rc"));
...
void KonqMainWindow::urlLoaderFinished(UrlLoader* loader)
{
    ...
    // Apply toggle view settings (e.g. sidebar) on the first URL load,
    // once a view exists, regardless of whether the load succeeded.
    if (m_bNeedApplyKonqMainWindowSettings && childView) {
        m_bNeedApplyKonqMainWindowSettings = false;
        ...
        const KParts::WindowArgs &windowArgs, KParts::ReadOnlyPart **part)
{
    ...
    if (!createTab && !browserArgs.forcesNewWindow() /* explicit "Open in New Window" action, e.g. on frame or history item */) {
        if (args.actionRequestedByUser()) { // MMB or some RMB popupmenu action
            createTab = HybridFMSettings::mmbOpensTab();
        } else { // Javascript popup
            createTab = HybridFMSettings::popupsWithinTabs() &&
    ...
    if (createTab && !m_isPopupWithProxyWindow) {
        ...
        if (HybridFMSettings::newTabsInFront()) {
            newtabsinfront = !newtabsinfront;
        }
        const bool aftercurrentpage = HybridFMSettings::openAfterCurrentPage();
...
void KonqMainWindow::slotHome()
{
    ...
    KonqOpenURLRequest req;
    req.browserArgs.setNewTab(true);
    req.newTabInFront = HybridFMSettings::newTabsInFront();
    ...
    if (modifiers & Qt::ControlModifier) { // Ctrl Left/MMB
        ...
        if (HybridFMSettings::mmbOpensTab()) {
            openFilteredUrl(homeURL, req);
        } else {
...
void KonqMainWindow::slotConfigure(const QString startingModule)
{
    ...
    if (!m_configureDialog) {
        ...
        const char *const toplevelModules[] = {
            "hybridfm_kcms/khtml_general",
#ifndef Q_OS_WIN
            "hybridfm_kcms/kcm_performance",
#endif
            "hybridfm_kcms/kcm_bookmarks",
        };
        ...
        m_configureDialog->addModule(KPluginMetaData(QStringLiteral("hybridfm_kcms/kcm_konq")));
        ...
        m_configureDialog->addModule(KPluginMetaData(QStringLiteral("hybridfm_kcms/khtml_behavior")));

        const char *const webModules[] = {
            "hybridfm_kcms/khtml_appearance",
            "hybridfm_kcms/khtml_filter",
            "kcm_webshortcuts",
            "kcm_proxy",
            "hybridfm_kcms/kcm_history",
...
void KonqMainWindow::slotConfigureSpellChecking()
{
    ...
    dialog.setWindowIcon(QIcon::fromTheme("hybrid-fm"));
...
{
    const QStringList toggableViewsShown = HybridFMSettings::toggableViewsShown();
...
void KonqMainWindow::slotPartActivated(KParts::Part *part)
{
    ...
    if (m_paHomePopup->text() == i18n("Home") || viewShowsDir != buttonShowsFolder) {
        ...
        actHomePage->setWhatsThis(i18n("<html>Navigate to your 'Home Page'<br /><br />"
                                       ...
                                       "under <b>Settings -> Configure HybridFM -> General</b>.</html>"));
        actHomePage->setData(HybridFMSettings::homeURL());
...
void KonqMainWindow::splitCurrentView(Qt::Orientation orientation)
{
    ...
    KSharedConfig::Ptr cfg = KSharedConfig::openConfig("hybrid-fmrc");
    ...
    if (!alwaysDuplicateView && !url.isLocalFile()) {
        url = QUrl(HybridFMSettings::startURL());
        if (url.isLocalFile()) {
            QMimeDatabase db;
...
void KonqMainWindow::slotAddTab()
{
    ...
    KonqView *newView = m_pViewManager->addTab(QStringLiteral("text/html"),
                        QString(),
                        false,
                        HybridFMSettings::openAfterCurrentPage());
    if (!newView) {
        return;
    }
...
{
    m_pViewManager->duplicateTab(m_pViewManager->tabContainer()->currentIndex(), HybridFMSettings::openAfterCurrentPage());
}

void KonqMainWindow::slotDuplicateTabPopup()
{
    m_pViewManager->duplicateTab(m_workingTab, HybridFMSettings::openAfterCurrentPage());
}
...
void KonqMainWindow::slotPopupNewTab()
{
    ...
    bool openAfterCurrentPage = HybridFMSettings::openAfterCurrentPage();
    bool newTabsInFront = HybridFMSettings::newTabsInFront();
...
void KonqMainWindow::slotUp()
{
    ...
    req.openAfterCurrentPage = HybridFMSettings::openAfterCurrentPage();
    req.newTabInFront = HybridFMSettings::newTabsInFront();
...
void KonqMainWindow::slotGoHistoryDelayed()
{
    ...
    bool openAfterCurrentPage = HybridFMSettings::openAfterCurrentPage();
    bool mmbOpensTab = HybridFMSettings::mmbOpensTab();
    bool inFront = HybridFMSettings::newTabsInFront();
...
void KonqMainWindow::slotCompletionModeChanged(KCompletion::CompletionMode m)
{
    ...
    HybridFMSettings::setSettingsCompletionMode(int(m_combo->completionMode()));
...
void KonqMainWindow::initActions()
{
    ...
    m_paLinkView = new KToggleAction(i18nc("This option links hybrid-fm views", "Lin&k View"), this);
    ...
    m_paStop->setWhatsThis(i18n("<html>Stop loading the document<br /><br />"
                                "All network transfers will be stopped and HybridFM will display the content "
    ...
    m_paCut->setWhatsThis(i18n("<html>Cut the currently selected text or item(s) and move it "
                               ...
                               "This makes it available to the <b>Paste</b> command in HybridFM "
...
void KonqMainWindow::updateOpenWithActions()
{
    ...
    const int baseOpenWithItems = qMax(HybridFMSettings::openWithItems(), 0);
...
bool KonqMainWindow::refuseExecutingKonqueror(const QString &mimeType)
{
    if (activeViewsNotLockedCount() > 0) {   // if I lock the only view, then there's no error: open links in a new window
        KMessageBox::error(this, i18n("There appears to be a configuration error. You have associated HybridFM with %1, but it cannot handle this file type.", mimeType));

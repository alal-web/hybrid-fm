...
void KonqView::slotCompleted(bool hasPending)
{
    ...
    if (!m_bGotIconURL && !m_bAborted) {
        if (HybridFMSettings::enableFavicon() == true) {
...
{
    if (HybridFMSettings::enableFavicon()) {
...
void KonqView::appendHistoryEntry(HistoryEntry *historyEntry)
{
    ...
    while (m_lstHistory.count() > 0 && m_lstHistory.count() >= HybridFMSettings::maximumHistoryEntriesPerView()) {

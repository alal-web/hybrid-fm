...
void ViewMgrTest::initTestCase()
{
    ...
    QCOMPARE(HybridFMSettings::mmbOpensTab(), true);
    QCOMPARE(HybridFMSettings::popupsWithinTabs(), false);
    HybridFMSettings::setAlwaysHavePreloaded(false); // it would confuse the mainwindow counting
...
void ViewMgrTest::testCtrlClickOnLink()
{
    ...
    if (HybridFMSettings::mmbOpensTab()) {
        ...
        if (HybridFMSettings::newTabsInFront()) { // when called by sameTestsWithNewTabsInFront
            QCOMPARE(tabs->currentIndex(), 1);
...
void ViewMgrTest::sameTestsWithNewTabsInFront()
{
    ...
    QVERIFY(!HybridFMSettings::newTabsInFront()); // default setting = false
    HybridFMSettings::setNewTabsInFront(true);
    testPopupNewTab();
    testCtrlClickOnLink();
    HybridFMSettings::setNewTabsInFront(false);
}
...
void ViewMgrTest::sameTestsWithMmbOpenTabsFalse()
{
    ...
    HybridFMSettings::setMmbOpensTab(false);
    testPopupNewTab();
    testPopupNewWindow();
    testCtrlClickOnLink();
    HybridFMSettings::setMmbOpensTab(true);
}
